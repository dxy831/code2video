...
{solution_code}
    ...
    1.  **多维布局策略 (Layout Strategy)**:
        ...
        - **强制分页规则 (Pagination Protocol)**:
          - **🔴 每行字数限制**: 每句 lecture_line 不超过 **20个中文字符**（含标点、英文字母、数字）即可放一行，无需刻意拆短。只有超过20字时才需要按语义断句拆成多行。**不要把一句完整的短句强行拆成两行！一句话能在20字以内说完就放一行。**
            - ❌ `"我们定义f[i][j]表示：s的前i个字符能否被p的前j个字符完全匹配。"` (34字，超过20字，必须拆行)
            - ✅ `"我们定义f[i][j]表示："` + `"s的前i个字符，能否被p的前j个字符完全匹配。"`
            - ❌ 把 `"接下来是：DP表的初始化。"` 拆成 `"接下来是："` + `"DP表的初始化。"` (原句只有12字，不需要拆！)
            - ✅ `"接下来是：DP表的初始化。"` (12字，不超过20字，直接放一行)
          - **🔴🔴🔴 分批行数限制（按有无代码块区分，这是最容易犯错的地方！）**:
            - **有代码块的章节**（代码精讲、示例模拟等 Split-Left Layout）：每批最多 **4行**
            - **无代码块的章节**（题目解读、思路分析、易错点、总结等 Left-Right Split）：每批最多 **8行**
            - **🔴 严禁不区分场景一律每批4行！思路分析等无代码章节必须用8行上限！**
          - **🔴🔴🔴 按语义完整性分批（比行数限制更重要！严禁机械凑满行数！）**:
            - **一个知识点的所有内容必须在同一批**，不能拆到两批
            - **不同知识点不能硬凑到同一批**
            - 如果一个知识点只有2行，就只显示2行；如果有6行，就显示6行
            - **严禁机械地每批都凑满4行！**
            - ❌ `["特征1标题", "特征1详解", "特征1总结", "特征2标题"]` — 特征2被硬塞进特征1的批次！
            - ✅ `["特征1标题", "特征1详解", "特征1总结"]` 为一批，`["特征2标题", "特征2详解"]` 为另一批
